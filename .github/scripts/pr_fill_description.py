...
- Use EXACTLY these sections in this order:
  ...
  2. `## Checklist` — only include items that actually apply to this diff:
     - `- [ ] New DB columns/tables have a migration in \\`crumb.elixpo/migrations/\\`` (only if D1 schema changed)
     - `- [ ] Rate-limit middleware on new public auth endpoints` (only if new crumb.elixpo/app/api/auth/** route added)
     - `- [ ] \\`./deploy.sh --worker build\\` clean` (only if crumb.elixpo/** changed)
     - `- [ ] \\`cargo fmt --all --check\\`, Clippy, and workspace tests clean` (only if Rust or Cargo files changed)
