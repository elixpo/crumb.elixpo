...
export const metadata = pageMetadata({ title: 'Profiles and Connections', description: 'Manage the services securely connected to your Crumb account.', path: '/profiles', noIndex: true })
...
export default async function ProfilesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!user) redirect('/login?return_to=%2Fprofiles')
  ...
  return <><SiteHeader /><main className="account-page"><div className="profile-intro"><div className="account-heading"><p className="kicker">Profiles and connections</p><h1>Your secure service layer.</h1><p>Connect providers once in the browser, then authorize each Crumb terminal without pasting private keys.</p></div><aside className="profile-pill"><img src={`${config().accountsOrigin}/api/avatar/${encodeURIComponent(user.id)}`} alt="" /><span><small>Signed in as</small><strong>{user.displayName}</strong><em>{user.email}</em></span></aside></div>
