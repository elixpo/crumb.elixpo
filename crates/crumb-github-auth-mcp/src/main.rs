//! GitHub (authenticated) MCP server.
...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("github-auth") { bail!("Unknown tool"); }
    ...
    let mut url = format!("https://api.github.com/repos/{}", repo);

    let token = keyring::Entry::new("crumb.elixpo", "github").and_then(|e| e.get_password()).unwrap_or_default();
    ...
    let output = { let stars = resp.get("stargazers_count").and_then(|v| v.as_u64()).unwrap_or(0); format!("Repo {}: {} stars", repo, stars) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-github-auth-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-github-auth-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "github-auth", "description": "Authenticated GitHub access",
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "github-auth", "arguments": {}});
