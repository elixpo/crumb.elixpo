...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("discord") { bail!("Unknown tool"); }
    ...
    let mut url = format!("https://discord.com/api/v10/channels/{}", channel);

    let token = keyring::Entry::new("crumb.elixpo", "discord").and_then(|e| e.get_password()).unwrap_or_default();
    ...
    let output = { let name = resp.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown"); format!("Channel name: {}", name) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-discord-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-discord-mcp", "version": "1.0.0"}}}),
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "discord", "arguments": {}});
