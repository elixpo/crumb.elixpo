...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("todoist") { bail!("Unknown tool"); }
    ...
    let mut url = format!("https://api.todoist.com/rest/v2/projects/{}", project_id);

    let token = keyring::Entry::new("crumb.elixpo", "todoist").and_then(|e| e.get_password()).unwrap_or_default();
    ...
    let output = { let name = resp.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown"); format!("Project name: {}", name) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-todoist-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-todoist-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "todoist", "description": "Todoist integration",
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "todoist", "arguments": {}});
