...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("notion") { bail!("Unknown tool"); }
    
    let args = params.get("arguments").ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    let page_id = args.get("page_id").and_then(|v| v.as_str()).unwrap_or("");

    let mut url = format!("https://api.notion.com/v1/pages/{}", page_id);

    let token = keyring::Entry::new("crumb.elixpo", "notion").and_then(|e| e.get_password()).unwrap_or_default();
    ...
    let output = { let url = resp.get("url").and_then(|v| v.as_str()).unwrap_or("Unknown"); format!("Page URL: {}", url) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-notion-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-notion-mcp", "version": "1.0.0"}}}),
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "notion", "arguments": {}});
