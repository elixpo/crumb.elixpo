...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("dictionary") { bail!("Unknown tool"); }
    ...
    let output = { let definition = resp.as_array().and_then(|a| a.get(0)).and_then(|o| o.get("meanings")).and_then(|a| a.as_array()).and_then(|a| a.get(0)).and_then(|o| o.get("definitions")).and_then(|a| a.as_array()).and_then(|a| a.get(0)).and_then(|o| o.get("definition")).and_then(|v| v.as_str()).unwrap_or("Definition not found"); format!("{}: {}", word, definition) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-dictionary-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-dictionary-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "dictionary", "description": "Define a word using dictionaryapi.dev",
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "dictionary", "arguments": {}});
