...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("shorturl") { bail!("Unknown tool"); }
    
    let args = params.get("arguments").ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    let url = args.get("url").and_then(|v| v.as_str()).unwrap_or("");

    let url = format!("https://is.gd/create.php?format=json&url={}", url);
    ...
    let output = { let shorturl = resp.get("shorturl").and_then(|v| v.as_str()).unwrap_or("Error"); format!("Short URL for {}: {}", url, shorturl) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-shorturl-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-shorturl-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "shorturl", "description": "Shorten a URL using is.gd",
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "shorturl", "arguments": {}});
