...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("crypto") { bail!("Unknown tool"); }
    ...
    let output = { let price = resp.get(&coin).and_then(|o| o.get("usd")).and_then(|v| v.as_f64()).unwrap_or(0.0); format!("Price of {}: ${}", coin, price) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-crypto-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-crypto-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "crypto", "description": "Check cryptocurrency price using CoinGecko",
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "crypto", "arguments": {}});
