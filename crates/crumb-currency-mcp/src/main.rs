...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("currency") { bail!("Unknown tool"); }
    ...
    let url = format!("https://api.frankfurter.app/latest?amount={}&from={from_cur}&to={to_cur}", amount, from_cur, to_cur);
    ...
    let output = { let rate: f64 = resp.get("rates").and_then(|r| r.get(&to_cur)).and_then(|v| v.as_f64()).unwrap_or(0.0); format!("{} {} = {} {}", amount, from_cur, rate, to_cur) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-currency-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-currency-mcp", "version": "1.0.0"}}}),
                    "ping" => json!({"jsonrpc": "2.0", "id": id, "result": {}}),
                    "tools/list" => json!({"jsonrpc": "2.0", "id": id, "result": {"tools": [{
                        "name": "currency", "description": "Convert currency using Frankfurter.dev",
                        "inputSchema": {"type": "object", "properties": {"amount": {"type": "string"}, "from": {"type": "string"}, "to": {"type": "string"}}, "required": ["amount", "from", "to"]},
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "currency", "arguments": {}});
