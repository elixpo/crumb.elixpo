...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("quote") { bail!("Unknown tool"); }
    
    let args = params.get("arguments").ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;

    let url = "https://zenquotes.io/api/random".to_string();
    ...
    let output = { let quote = resp.as_array().and_then(|a| a.get(0)).and_then(|o| o.get("q")).and_then(|v| v.as_str()).unwrap_or(""); let author = resp.as_array().and_then(|a| a.get(0)).and_then(|o| o.get("a")).and_then(|v| v.as_str()).unwrap_or(""); format!("\"{}\" - {}", quote, author) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-quote-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-quote-mcp", "version": "1.0.0"}}}),
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "quote", "arguments": {}});
