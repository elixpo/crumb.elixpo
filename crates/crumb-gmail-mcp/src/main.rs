//! Gmail (search/draft only) MCP server.
...
fn handle_call(params: &Value) -> Result<Value> {
    ...
    if name != Some("gmail") { bail!("Unknown tool"); }
    
    let args = params.get("arguments").ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    let query = args.get("query").and_then(|v| v.as_str()).unwrap_or("");

    let mut url = format!("https://gmail.googleapis.com/gmail/v1/users/me/messages?q={}", query);

    let token = keyring::Entry::new("crumb.elixpo", "gmail").and_then(|e| e.get_password()).unwrap_or_default();
    ...
    let output = { let msgs = resp.get("messages").and_then(|v| v.as_array()).map(|a| a.len()).unwrap_or(0); format!("Found {} messages", msgs) };
        ...
        if let Some(obj) = value.as_object() {
            if let Some(method) = obj.get("method").and_then(|m| m.as_str()) {
                ...
                let response = match method {
                    "server/discover" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2026-07-28", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-gmail-mcp", "version": "1.0.0"}}}),
                    "initialize" => json!({"jsonrpc": "2.0", "id": id, "result": {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}}, "serverInfo": {"name": "crumb-gmail-mcp", "version": "1.0.0"}}}),
...
mod tests {
    ...
    fn test_handle_call_missing_args() {
        let params = json!({"name": "gmail", "arguments": {}});
